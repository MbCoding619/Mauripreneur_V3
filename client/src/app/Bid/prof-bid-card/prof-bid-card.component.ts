...
import { Bid } from 'src/app/_models/Bid';
...
export class ProfBidCardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.checkBidStatus(this.params);
  ...
  checkBidStatus(jobId: any) {
    this.bidService.checkBidStatus(jobId).subscribe(response => {
      this.checkBid = response;
      this.checkBidL = Object.keys(this.checkBid).length;
      console.log(this.checkBidL);
    })
  }
